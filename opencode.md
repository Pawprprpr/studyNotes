AI Agent 企业级实战知识骨架
基于 Java 专家视角构建的系统性学习框架 目标：能设计并实现类似于 OpenCode、Cursor 的企业级 AI 编程助手，并覆盖 CI 自动修复等场景
...
工具定义层（tool/read.ts 等）：定义工具的 description、parameters、execute 方法
工具注册层（tool/registry.ts）：统一注册管理，支持动态发现和插件加载
工具包装层（session/prompt.ts）：在 execute 外层包装插件扩展点（before/after）
扩展点设计：
...
命令白名单：只允许特定命令（rm、cp、mv、cat 等）
路径过滤：检查文件是否在允许目录
动态权限请求：执行前调用 ctx.ask() 询问用户
Agent 级别配置：不同 Agent 有不同权限（build 全权限、plan 禁用 edit 等）
Deferred 机制（等待用户回复）：
